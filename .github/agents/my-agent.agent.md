...
本项目是 [PKUS XNY Ultra](https://github.com/c-jeremy/pkus-xny-ultra) 的 Fork，为北京大学新能源课程系统提供增强脚本，适配 Via 浏览器，同时兼容 Edge 浏览器（含安卓版）。所有脚本均为独立的 Tampermonkey 用户脚本（`.user.js`），可按需单独安装。

主要功能包括：路径收藏夹、目录搜索、强制下载 PDF、硬刷新、移除无用界面元素、自动收起侧边栏、图片/相机上传选择、手写滑动修复、强制显示答案、纯黑白模式等。

### 目录约定

- `vv/`：当前活跃版本，所有开发工作在此进行。
- `v1/`、`v2/`：已废弃的旧版本，除非明确提到，否则不需要关注。
- `README.md`：主 README，需要与 `vv/` 内实际脚本保持一致（尤其是脚本列表）。
