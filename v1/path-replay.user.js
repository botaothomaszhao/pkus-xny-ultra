...
// @description  修复了按钮不显示的致命bug。确保按钮第一时间无条件加载，并智能地在导航栏出现后附加高级保存功能。
...
(function() {
    ...
    async function replayPath(path) {
        ...
        // 新增：回放成功后，点击 class 为 "put" 的元素

        /*console.log("🎯 尝试点击 class 为 'put' 的元素...");

        await new Promise(r => setTimeout(r, 500)); // 稍等一下确保页面稳定
        ...
        if (putElement) {
            ...
            console.log("✅ 成功点击了 class 为 'put' 的元素");

            GM_notification({ title: '路径回放完成', text: '已成功恢复到记忆的位置并执行了额外操作。', timeout: 3000 });

        } else {

            console.log("⚠️ 未找到 class 为 'put' 的元素");

            GM_notification({ title: '路径回放完成', text: '已成功恢复到记忆的位置。', timeout: 3000 });
    ...
    const container = document.createElement('div'); container.id = 'replay-path-container';

    const button = document.createElement('button'); button.id = 'replay-path-btn';

    button.title = '一键回放至上次记录的路径';

    const replayIcon = document.createElement('div'); replayIcon.className = 'replay-icon';

    replayIcon.innerHTML = `<svg viewBox="0 0 24 24"><path d="M8 5v14l11-7z"></path></svg>`;

    button.appendChild(replayIcon); container.appendChild(button); document.body.appendChild(container);
    ...
    button.addEventListener('click', async () => {
        ...
        container.classList.add('loading'); button.disabled = true;
        ...
        } catch (error) { console.error('回放失败:', error); }

        finally { container.classList.remove('loading'); button.disabled = false; }
