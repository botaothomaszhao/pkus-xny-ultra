...
| `page-cleaner.user.js`       | 页面清理    | 删除页面中的无用元素、使标签栏可滑动。                       | 通用                   |                                                                     |
| `unified-modal.user.js`      | 统一弹窗    | 统一不同类型的弹窗样式。                              | 通用                   | 使全部适配弹窗（如PDF预览）支持全屏、点击周围关闭。                                         |
| `responsive-sidebar.user.js` | 自动收起侧边栏 | 在屏幕宽度改变时自动收起/展开侧边栏，并可通过点击左侧导航菜单展开。    | 通用                   |                                                                     |
| `image-selector.user.js`     | 图片选择框   | 上传照片时选择拍照上传或相册上传，拍照唤起系统相机，可禁用增强以加速上传。 | 通用                   | 完成拍照后可使用新能源自带旋转、裁剪工具。                                               |
| `build-in-camera.user.js`    | 网页内置相机  | 在网页内实现相机拍照，解决浏览器无法唤起相机的问题。            | Via等不支持capture字段的浏览器 | 与图片选择框脚本配合会作为“拍照上传”选项，单独使用会对全部图片上传元素生效。                             |
| `handwriting-fix.user.js`    | 手写滑动修复  | 修复手写输入时窗口上下滑动问题，支持查看题干同时作答。           | 通用                   | 仅Edge安卓版有窗口滑动问题。                                                    |
| `show-answer.user.js`        | 强制显示答案  | 强制显示题目上被隐藏的“答案”等按钮。                   | 通用                   | 需要确保运行顺序早于其他具有请求监控等功能的脚本。开启即表示你完全对你的行为负责。                           |
...
- [x] 测试改用ImageCapture
    ...
    - [ ] 是否需要缩放功能？
- [x] 从v2移植强制显示答案功能
    - [x] 修复和其他脚本冲突 (需要强制显示答案先运行)
- [x] 关闭科目后清除url参数以避免路径保存错误
- [x] 课程页顶部标签栏从只能用两侧按键移动改为可滑动
- ~~[ ] 研究错题重做相关功能~~ 后端不支持
- [x] 图片选择框添加禁用增强选项（从v2获取实现代码）
    - [ ] 修改UI布局
- [x] 页面回退后如已展开则不重放路径
- [x] 手写作答支持查看题目同时手写
    - [x] 尝试在使用触摸板时忽略触摸、支持单击
    - [x] 题干背景滚动
    - [x] 添加题干滚动按钮
    - [x] 修复没有书写笔迹时卡死
    - [x] 有手写笔时用触摸滑动背景
