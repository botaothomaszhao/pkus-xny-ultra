...
| `del-unused.user.js`           | 删除无用元素    | 自动删除无用页面元素，包括头部“学科素养”、“考试用时”和未提交时的空图片框。 | 通用                      |                                                                     |
| `responsive-sidebar.user.js`   | 自动收起侧边栏   | 在屏幕宽度改变时自动收起/展开侧边栏，并可通过点击左侧导航菜单展开。      | 通用                      |                                                                     |
| `image-upload-custom.user.js`  | 上传照片-内置相机 | 上传照片时选择相机上传或拍照上传，相机拍照在网页内实现             | Via等不支持capture字段的浏览器    | 实际上完全通用，但浏览器支持的情况下推荐capture版，因为系统相机通常更强大、高质。                        |
| `image-upload-capture.user.js` | 上传照片-系统相机 | 上传照片时选择相机上传或拍照上传，拍照唤起系统相机。              | Edge安卓版等支持capture字段的浏览器 | 完成拍照后可使用新能源自带旋转、裁剪工具。                                               |
...
- [x] 添加为Edge的拍照上传脚本
    ...
    - [x] 固定input的accept属性
    - [ ] 考虑如何复用代码
- [x] 修复强制刷新选中使用Enter键不生效
- [x] 修复拍照界面X按钮跑到屏幕外
- [x] 干掉“扫一扫传答案”
- [ ] 解决ItemDrawer focus和高光不一致
- [ ] 修复分数栏可能被顶部top元素遮挡
